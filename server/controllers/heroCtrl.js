...
module.exports = {
  ...
  getHeroes: function(req, res) {
    db.hero.read_heros([], function(err, results) {
      if (err) {
        console.error(err);
        return res.send(err);
      }
  ...
  getHero: function(req, res) {
    db.hero.read_hero([req.params.heroId], function(err, results) {
      ...
      if (results.length === 0) {
        return res.status(404).send("No hero found... You might need to add one in")
      }
      else {
        res.send(results);
      }
  ...
  delete: function(req, res) {
    ...
    function(err, results){
      ...
      if (results.length === 0) {
        ...
        //Or, to show name, res.send(results[0].name + ' is dead');
      }
    })
  }
}
